...
> The unnest_tokens is tokenizing each word, so all of the text is now split into single words. The anti_join is being used to remove any stop words. 
...
tidy_sotu %>%
  ...
  arrange(desc(avg_words))
```
> It was not due to the stop words. The presidents tended to be ranked in the same position, but with a larger word count. 
