...
tidy_sotu %>%
  ...
  arrange(desc(avg_words))
```
> It was not due to the stop words. The presidents tended to be ranked in the same position, just with a larger word count. 
...
>  Count is counting each word, essentially assigning 1 value to each word.Group_by is just grouping the data by the president column. Mutate(n_tot = sum(n),is creating a new column that has the total word count in each doc.Term_freq = n/n_tot) is determining the frequency of a word in relation to the total word count. So John Adam used united 49 times. 49 is then divided by the total word count (2768), which produces the term frequency value (0.017702312). Arrange(desc(term_freq)) is displaying the term frequency from most signifiacnt to least. Top_n(1) is selecting the top rows by value for each president. Print(n = Inf) is then printing all of the rows. 
...
bb_word_pairs
```
> this is an interesting output, becuase it inidcates the pairing of words. Many of the pairings involve wild. So wild and indian, wild and west, wild and world, wild and life are some of the major pairing of words. These pairings may allude to the type of imagined place that Buffalo Bill wanted to convey to his audience, which is the west is wild and full of wild Native Americans. 
