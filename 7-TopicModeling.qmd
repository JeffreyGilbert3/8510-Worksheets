...
```{r}
epbb.lda <- LDA(epbb.dtm, k = 12, control = list(seed = 53345))
epbb.lda
```
...
```{r}
lpbb.lda <- LDA(lpbb.dtm, k = 12, control = list(seed = 62345))
lpbb.lda
```
...
```{r}
erbb.lda <- LDA(erbb.dtm, k = 12, control = list(seed = 72345))
erbb.lda
```
...
```{r}
lrbb.lda <- LDA(lrbb.dtm, k = 12, control = list(seed = 82345))
lrbb.lda
```
