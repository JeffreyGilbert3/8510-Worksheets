...
> The paste function essentially combined two seperate strings into one single string. So, the function ran both arguments and outputted those into a single string. It does, however, create a space between the two strings.
...
>5 == 10: This code indicates that 5 equals 10, which would return the value of FALSE.
3 < 4: This code indicates that 3 is less than 4, which would return the value of TRUE.
5 != 10: This code indicates that 5 does not equal 10, which would return the value of TRUE.
5 != 5 : This code indicates that 5 does not equal 5, which would return the value of FALSE.
3 == 4 | 3: This code indicates that 3 equals 4 or 3, which would return the value of TRUE.
3 != 4 & 3 != 5: This code indicates that 3 does not equal 4 and that 3 does not equal 5, which would return the value of TRUE.
...
> my.numbers was assigned a list of numbers. When my.numbers was multiplied by 2, all of the numbers in the list were multiplied by 2. This is due to my.numbers being a vector that is storing multiple values. 
...
> The [3] indicates that it is selecting the third value in the list for my.numbers. So when multiplying my.numbers[3] by 2 is really multiplying 6 by 2, since 6 is the third number in the list. 

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
Ingredients <- c("lettuce", "onions", "tomatos", "bread", "cheese")
```
...
>The floor function takes a single number (argument) and then retuns a number (vector) that contains the largest integers that are less than the elements of x.
...
>Row z is essenitally indicating that the value of z is equal to the value of x times 2 plus the value of y. So z is equal to 1:5 * 2 + 1, since x equals 1:5 and y equals 1.
...
> The documentation indicates that the 'gayguides' dataset is a compilation of Bob Damron's Address Books (1965-1985). The documentation also mentions that the dataset was last updated in December 2022. 
...
>I used ?head to bring up the help panel. Within the help panel were instructions about how to use the tail function. With regard to printing out 10 rows instead of 6, I ended up using Google. The websites provided the information in a more digestable manner than the help panel. 
...
>It seems like the elements kind of act like a directory, in the sense that gayguides, $, title, and [] serve as the pathway to retrieve the data so that the data can then be outputted:
gayguides: is the name of the dataset. 
$: is the operator that allows the user to retrieve the data from a specific variable.
title: is the name of the variable that $ is drawing from.
[500]: is the 500th value in the list for the title variable, which happens to be a "Rest Stop" 
...
```
> There are 10702 observations in the "BostonWomenVoters" dataset. 

```{r}
head(BostonWomenVoters$Age)
```
```{r}
tail(BostonWomenVoters$Age)
```
```{r}
median(BostonWomenVoters$Age)
```
```{r}
BostonWomenVoters$Precinct[250]
```

(@) What did you learn about the dataset? What can you tell me about it? 

>The "BostonWomenVoters"dataset has 34 variables with a range of topics. What is interesting though is that the dataset has information about the womens' fathers and husbands, but nothing about their mothers. It seems strange that the creators of this dataset would omit the womens' mother's information and only include information about the womens' male relatives. There is also no information available for the women's age at the tail of the vector. 
